...
export default {
  ...
  data() {
    var validateName = (rule, value, callback) => {
      if (this.editType == "add" || this.name1 != value) {
        if (!!value) {
          this.postRequest(
            "/api/" + this.serviceName + "/jrr/errorCode_biz/name",
            {
              name: value
            }
          ).then(result => {
    ...
    var validateCode = (rule, value, callback) => {
      if (this.editType == "add" || this.code1 != value) {
        if (!!value) {
          if (value < 1000) {
            callback(new Error("请输入1000以上编码"));
          } else {
            this.postRequest(
              "/api/" + this.serviceName + "/jrr/errorCode_biz/code",
              {
                code: value
              }
            ).then(result => {
  ...
  methods: {
    ...
    getData() {
      this.loading = true;
      this.getRequest("/api/ts-errorcode/error/code/biz/select/all").then(
        result => {
    ...
    addSubmits() {
      ...
      SubmitForm(
        ...
        "studentForm",
        this.editType,
        "/api/" + this.serviceName + "/cor/errorCode_biz",
        "/api/" + this.serviceName + "/uor/errorCode_biz",
        function() {
          that.getData();
    ...
    deleteRow(index, data) {
      ...
      DeleteStatus(
        this,
        "/api/" + this.serviceName + "/dor/errorCode_biz",
        {
          id: data.id
        },
        function() {
