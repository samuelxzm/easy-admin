<template>
	<el-card style="min-height:100%;box-sizing:border-box;border-radius:0;">
		...
		<el-dialog title="代码项目管理增加" :visible.sync="addDialog" :close-on-click-modal="false">
			...
			<el-form :rules="rules" :model="studentForm" label-width="120px" ref="studentForm" size="small">
				<!-- status-icon -->
				<el-row>
					<!--<el-col :span="12">
					...
					<el-col :span="12">
						<el-form-item label="类型" prop="type">
							<el-select v-model="studentForm.type"  placeholder="请选择项目" style="width: 100%;">
								<el-option v-for="item in GitTypeOption" :key="item.id" :label="item.name" :value="item.code"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="git仓库" prop="gitRepositoryId">
							<el-select v-model="studentForm.gitRepositoryId"  placeholder="请选择项目" style="width: 100%;">
								<el-option v-for="item in GitMoldOption" :key="item.id" :label="item.name" :value="item.id"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script>
	...
	import { SubmitForm, guid, DeleteStatus, moldStatus } from "@/assets/js/common.js";
	export default {
		...
		data() {
			var validateName = (rule, value, callback) => {
				...
				if(this.editType == "add") {
					if(!!value) {
						this.postRequest("/api/" + this.serviceName + "/jrr/codeProject/name", {
							name: value
						}).then(result => {
							if(!result.data) {
								callback();
		...
		methods: {
			...
			getData() {
				this.loading = true;
				this.getRequest("/api/ts-git-code/code/project/get/all").then(
					result => {
			...
			addSubmit(type, data) {
				...
				if(type == "edit") {
					...
					this.studentForm = {
					    ...
					    sortNo: "100",
						gitName:"",
						gitRepositoryId: this.GitMoldOption.length>0?this.GitMoldOption[0].id:'',
			...
			getGitType() {
				var that = this;
				that.getRequest("/api/" + this.serviceName + "/gar/dic_codeType").then(result => {
			...
			getGitMold() {
				var that = this;
				that.getRequest("/api/" + this.serviceName + "/gar/gitRepository").then(result => {
